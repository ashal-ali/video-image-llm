...
class BaseDataLoader(DataLoader):
    ...
    """
    def __init__(self, dataset, batch_size, shuffle, validation_split, num_workers, collate_fn=default_collate,prefetch_factor=2, use_ddp=False):
        self.validation_split = validation_split
        self.shuffle = shuffle
        self.batch_idx = 0
        self.n_samples = len(dataset)
        if use_ddp:
            self.sampler, self.valid_sampler = self._split_sampler_ddp(self.validation_split)
    ...
    def _split_sampler_ddp(self, split):
        if split == 0.0:
            return None, None
...
class BaseDataLoaderExplicitSplit(DataLoader):
    ...
    Base class for all data loaders
    """
    def __init__(self, dataset, batch_size, shuffle, num_workers, collate_fn=default_collate, prefetch_factor=2, sampler=None):
        self.shuffle = shuffle
        self.batch_idx = 0
        self.n_samples = len(dataset)
